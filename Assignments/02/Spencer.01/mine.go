...
func MineBlock(bk *block.BlockType, difficulty string) {
	...
	for {
		...
		theHash := hash.HashOf(serializedBlock)
		theHashAsAString := hex.EncodeToString(theHash)
		fmt.Printf("((Mining)) Hash for Block [%s] nonce [%8d]\r", theHashAsAString, bk.Nonce)
		if theHashAsAString[0:4] == "0000" {
			// PJS: bk.Seal = theHashAsAString
			// The underlying type in block.Blocktype (.Seal) is []byte, this is the same as
			// theHash returned fro hash.HashOf(...) - we only need the string so that we can
			// check the "if" above.
			bk.Seal = theHash
			fmt.Printf("((Mining)) Hash for Block [%s] nonce [%8d]\n", theHashAsAString, bk.Nonce)
			return

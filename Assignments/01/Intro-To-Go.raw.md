...
In Windows you should be in your home directory or a similar directory.  In the bash shell from git 
...
or on Windows using the `bash` shell that is installed with git
...
or on Windows using the `powershell`
...
You only need to do the `go mod init` once in a directory, before the 1st `go build`.
...
2. Change the `fmt.Pr...` to `fmt.Println("Hello, 世界")` - re run the code and save the output.
	What is `世界`?
3. Why is it important that you can use Unicode directly in your Go program?
4. <b>IMPORTANT!</b> check that your name is in the comments in your source code.
4. <b>IMPORTANT!</b> do not turn in the executable!  It will be large.   When you get done with all 9
sections of the homework use tar on Mac or Linux or Zip on windows to collect all the sections of
the homework.  Delete all the executable first - then upload the single file with all 9 sections
in it.
...
Let's build this in the `echo1` directory at the same level as `hellow-world`, so
...
Initialize the project in the echo2 directory.


Edit main.go and put this code in it:

```Go
m4_include(echo2/echo2.go)
```

Create a 2nd file - call it cfg.json with:

```JSON
m4_include(echo2/cfg.json)
```
...
Create a directory `read-json1` to for this section.
...
Create a directory `mkPkg` to for this section.
...
```
	...
	import (
		...
		"github.com/...some-path.../myPkg/test1" // import package you created
	)
...
Note that ...some-path... is whatever path you are using for this.  It could be 
`Univ-Wyo-Education/S21-4010/Assignments/01` if you did your homework in a checked out copy of
the class.
...
Put this into a directory, `ksum`.
...
[https://github.com/Univ-Wyo-Education/S21-4010/tree/master/Assignments/01/file1](https://github.com/Univ-Wyo-Education/S21-4010/tree/master/Assignments/01/file1),
[https://github.com/Univ-Wyo-Education/S21-4010/tree/master/Assignments/01/file2.txt](https://github.com/Univ-Wyo-Education/S21-4010/tree/master/Assignments/01/file2.txt),
[https://github.com/Univ-Wyo-Education/S21-4010/tree/master/Assignments/01/file3](https://github.com/Univ-Wyo-Education/S21-4010/tree/master/Assignments/01/file3) . Save
...
- Windows Note: You need to change the behavior of the windows exporter to show extensions.  In windows 10 look for the developer configuration and turn that on.
- Windows Note: It is international that file1 and file3  do not have any extension.
...
9. Cross compile for Linux and different architectures like Web Assembler is built in.
